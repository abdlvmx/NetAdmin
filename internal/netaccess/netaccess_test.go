...
//
// Прежде явный список отсекал и её, и это стоило дорого. Администратор,
// сузивший доступ до подсети организации, терял интерфейс на самом сервере —
// вернуть его можно было только правкой config.json руками на машине.
// Заодно переставал работать агент, поставленный кнопкой «на этот
// компьютер»: он ходит на 127.0.0.1.
//
// Ничего это не закрывало: тот, кто пришёл через петлю, уже на машине и
// может читать базу, править настройки и останавливать службу.
func TestLoopbackAllowedWhateverTheList(t *testing.T) {
	l, err := Parse("192.168.1.0/24")
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	for _, ip := range []string{"127.0.0.1", "127.0.0.5", "::1", "::ffff:127.0.0.1"} {
		if !l.Allows(ip) {
			t.Errorf("петля %s отвергнута: администратор на самом сервере остался бы без интерфейса", ip)
		}
	}
	// и это не значит «разрешено всё»
	if l.Allows("192.168.2.1") {
		t.Error("посторонняя подсеть прошла")
	}
}
